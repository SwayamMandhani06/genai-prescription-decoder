    ...
    <div className="min-h-screen bg-canvas text-theme-primary flex flex-col font-sans transition-colors">
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-10 space-y-12">
        ...
        <section className="space-y-4">
          ...
          <h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight text-theme-primary leading-tight">
          ...
          <div className="p-4 sm:p-5 rounded-2xl bg-amber-500/10 border border-amber-500/20 text-theme-primary space-y-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
            <div className="p-4 rounded-xl bg-surface border border-theme">
              <div className="text-xs font-medium text-theme-muted">Evaluation protocol</div>
              <div className="text-xl font-bold text-theme-primary mt-1">1,200 Scripts (Target)</div>
              <div className="text-xs text-theme-secondary mt-0.5">Proposed multi-center study protocol; synthetic/demo data currently used</div>
            </div>
            <div className="p-4 rounded-xl bg-surface border border-theme">
              <div className="text-xs font-medium text-theme-muted">Baseline framework</div>
              <div className="text-xl font-bold text-theme-primary mt-1">OCR vs VLM vs Grounded</div>
              <div className="text-xs text-theme-secondary mt-0.5">Evaluation criteria comparing OCR, general VLMs, and grounded pipeline</div>
            </div>
            <div className="p-4 rounded-xl bg-surface border border-theme">
              <div className="text-xs font-medium text-theme-muted">Safety policy</div>
              <div className="text-xl font-bold text-theme-primary mt-1">Selective Abstention</div>
              <div className="text-xs text-theme-secondary mt-0.5">Flags ambiguous handwriting for clinician review rather than guessing</div>
            </div>
          </div>
        </section>
        ...
        <section className="space-y-4">
          ...
          <div className="rounded-xl border border-theme overflow-x-auto bg-surface">
            <table className="w-full text-left text-sm">
              <thead className="bg-surface-subtle text-xs font-semibold text-theme-muted border-b border-theme">
                <tr>
                  <th className="py-3.5 px-4 sm:px-6">Evaluation Metric</th>
                  <th className="py-3.5 px-4 text-theme-secondary">Conventional OCR (Simulated Baseline)</th>
                  <th className="py-3.5 px-4 text-theme-secondary">Zero-Shot VLM (Simulated Baseline)</th>
                  <th className="py-3.5 px-4 text-teal-700 dark:text-teal-300 font-bold bg-teal-50/50 dark:bg-teal-950/20">
                    AURA-Rx (Design Target)
                  </th>
                  <th className="py-3.5 px-4 sm:px-6 text-emerald-600 dark:text-emerald-400">Target Objective</th>
                </tr>
              </thead>
                ...
                {BENCHMARK_METRICS.map((metric) => (
                  <tr key={metric.id} className="hover:bg-surface-subtle/50 transition-colors">
                    <td className="py-4 px-4 sm:px-6">
                      <div className="font-semibold text-theme-primary">{metric.metric}</div>
                      <div className="text-xs text-theme-muted mt-0.5">{metric.definition}</div>
                    </td>
                    <td className="py-4 px-4 text-xs font-mono">{metric.traditionalOcr}</td>
                    <td className="py-4 px-4 text-xs font-mono">{metric.generalVlm}</td>
                    <td className="py-4 px-4 text-xs font-mono font-bold text-teal-700 dark:text-teal-300 bg-teal-50/50 dark:bg-teal-950/20">
                      {metric.auraRxModel}
                    </td>
                    <td className="py-4 px-4 sm:px-6 text-xs font-medium text-emerald-600 dark:text-emerald-400">
                      {metric.improvement}
                    </td>
                  </tr>
                ))}
        ...
        <section className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="p-5 rounded-xl bg-surface border border-theme space-y-2">
        ...
        <section className="space-y-4">
          ...
          <div className="rounded-xl border border-theme bg-surface divide-y divide-theme">
            {LASA_ERROR_ANALYSIS_PAIRS.map((pair) => (
              <div key={pair.id} className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="space-y-1">
                  <div className="text-xs font-mono font-bold text-theme-primary tracking-wide">
                    {pair.pair}
                  </div>
                ...
                <div className="flex items-center gap-3 shrink-0">
                  <span className="text-[11px] font-mono px-2 py-0.5 rounded bg-surface-subtle text-theme-muted border border-theme">
                    {pair.metricDistance}
                  </span>
                  <span className="text-xs font-semibold text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-950/30 px-2 py-0.5 rounded border border-amber-200 dark:border-amber-800">
        ...
        <section className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {DATASET_EVALUATION_SUMMARY.map((ds) => (
              <div key={ds.name} className="p-5 rounded-xl bg-surface border border-theme space-y-2">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <h3 className="text-sm font-bold text-theme-primary">{ds.name}</h3>
                    {ds.statusTag && (
                      <span className="inline-block mt-0.5 text-[10px] font-medium px-2 py-0.5 rounded bg-surface-subtle border border-theme text-theme-muted">
                        {ds.statusTag}
                      </span>
                    )}
                  </div>
                  <span className="text-xs font-mono px-2 py-0.5 rounded bg-surface-subtle border border-theme text-theme-secondary shrink-0">
                    {ds.sampleCount}
                  </span>
                </div>
                <p className="text-xs text-theme-secondary">{ds.origin}</p>
                <div className="text-[11px] text-theme-muted pt-1 border-t border-theme">
        ...
        <section className="p-6 rounded-2xl bg-surface-subtle border border-theme space-y-3">
